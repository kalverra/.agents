...
def main():
    ...
    pr_scores = [v.get("avg_score") for v in pr_history.values() if v.get("avg_score") is not None]
    pr_total = sum(pr_scores) / len(pr_scores) if pr_scores else 0

    main_scores = [v.get("avg_score") for v in main_history.values() if v.get("avg_score") is not None]
